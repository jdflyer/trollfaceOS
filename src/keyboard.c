...
uint8_t keyboardUpperScancodesTable[128] =
{
    0,  27, '!', '@', '#', '$', '%', '^', '&', '*',	/* 9 */
  '(', ')', '_', '+', '\b',	/* Backspace */
  '\t',			/* Tab */
...
__attribute__((interrupt)) void keyboardInterruptHandler(interrupt_frame_t* frame) {
    ...
    if (keyboardScancodesTable[scancode]!=0) {
        if((shiftPressed==false&&capsLock==false)||(shiftPressed==true&&capsLock==true)) {
            tputchar(&mainterm,keyboardScancodesTable[scancode]);
        }else{
            tputchar(&mainterm,keyboardUpperScancodesTable[scancode]);
        }
    }
    PICSendEOI(1);
}